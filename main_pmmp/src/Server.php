...
class Server{
	...
	private int $chunkSendBudgetScaleTick = -1;
	private int $chunkSendBudgetScale = 100;
	private int $chunkSendBudgetPending = 0;
	private int $chunkSendBudgetAsyncBacklog = 0;
	...
	private int $chunkPopulationPrefetchActive = 0;
	...
	public function getChunkSendBudgetScale() : int{
		if($this->chunkSendBudgetScaleTick === $this->tickCounter){
			return $this->chunkSendBudgetScale;
		}

		$online = count($this->playerList);
		$pending = 0;
		$requestedSending = 0;
		$populationPrefetchStarted = 0;
		$populationPrefetchResolved = 0;
		$populationPrefetchStale = 0;
		$populationPrefetchActive = 0;
		foreach($this->playerList as $player){
			$pressure = $player->getChunkRequestPressure();
			$pending += $pressure['pending'];
			$requestedSending += $pressure['requested_sending'];
			$populationPrefetchStarted += $pressure['population_prefetch_started'];
			$populationPrefetchResolved += $pressure['population_prefetch_resolved'];
			$populationPrefetchStale += $pressure['population_prefetch_stale'];
			$populationPrefetchActive += $pressure['population_prefetch_active'];
		}

		$asyncBacklog = $this->asyncPool->getTotalTaskQueueSize();
		$asyncSize = max(1, $this->asyncPool->getSize());
		$useAverage = array_sum($this->useAverage) / count($this->useAverage);
		$scale = 100;
		if($useAverage > 0.85 || $asyncBacklog > $asyncSize * 3 || ($online > 0 && $requestedSending > $online * 8)){
			$scale = 50;
		}elseif($useAverage > 0.70 || $asyncBacklog > $asyncSize || ($online > 0 && $requestedSending > $online * 4)){
			$scale = 75;
		}elseif($online > 0 && $useAverage < 0.45 && $asyncBacklog === 0 && $pending > $online * 24){
			$scale = 125;
		}

		$this->chunkSendBudgetScaleTick = $this->tickCounter;
		$this->chunkSendBudgetScale = $scale;
		$this->chunkSendBudgetPending = $pending;
		$this->chunkSendBudgetAsyncBacklog = $asyncBacklog;
		$this->chunkPopulationPrefetchStarted = $populationPrefetchStarted;
		$this->chunkPopulationPrefetchResolved = $populationPrefetchResolved;
		$this->chunkPopulationPrefetchStale = $populationPrefetchStale;
		$this->chunkPopulationPrefetchActive = $populationPrefetchActive;
		return $scale;
	}

	/**
	 * @phpstan-return array{scale: int, pending: int, async_backlog: int, population_prefetch_started: int, population_prefetch_resolved: int, population_prefetch_stale: int, population_prefetch_active: int}
	 */
	public function getChunkSendBudgetPressure() : array{
		$this->getChunkSendBudgetScale();
		return [
			'scale' => $this->chunkSendBudgetScale,
			'pending' => $this->chunkSendBudgetPending,
			'async_backlog' => $this->chunkSendBudgetAsyncBacklog,
			...
			'population_prefetch_active' => $this->chunkPopulationPrefetchActive,
		];
	}
	...
	private function startupPrepareConnectableNetworkInterfaces(
		...
		$prettyIp = $ipV6 ? "[$ip]" : $ip;
		try{
			$rakLib = new RakLibInterface($this, $ip, $port, $ipV6, $packetBroadcaster, $entityEventBroadcaster, $typeConverter);
			$rakLibRegistered = $this->network->registerInterface($rakLib);
			if($rakLibRegistered){
				$rakLib->setPacketLimit(max(1, $this->configGroup->getPropertyInt(Yml::NETWORK_RAKLIB_PACKET_LIMIT, 200)));
			}
	...
	public function prepareBatch(string $buffer, Compressor $compressor, ?bool $sync = null, ?TimingsHandler $timings = null) : CompressBatchPromise|string{
		...
		try{
			...
			if($threshold === null || $bufferLength < $threshold){
				...
				if(!$sync && $bufferLength >= $this->networkCompressionAsyncThreshold){

...
class LoginPacketHandler extends PacketHandler{
	...
	protected function mapSelfSignedTokenBody(array $bodyArray) : SelfSignedJwtBody{
		if(isset($bodyArray["identity"]) && is_string($bodyArray["identity"])){
			if(!isset($bodyArray["leguuid"])){
				$bodyArray["leguuid"] = $bodyArray["identity"];
			}
			if(!isset($bodyArray["mid"])){
				$bodyArray["mid"] = $bodyArray["identity"];
			}
			unset($bodyArray["identity"]);
		}
		$mapper = $this->defaultJsonMapper("OpenID JWT body");
		try{

...
class InGamePacketHandler extends PacketHandler{
	...
	public function handlePlayerAuthInput(PlayerAuthInputPacket $packet) : bool{
		...
		if($this->forceMoveSync && $hasMoved){
			...
			$dx = $newPos->x - $curPos->x;
			$dy = $newPos->y - $curPos->y;
			$dz = $newPos->z - $curPos->z;
			if(($dx * $dx + $dy * $dy + $dz * $dz) > 1){  //Tolerate up to 1 block to avoid problems with client-sided physics when spawning in blocks
	...
	private function handleUseItemTransaction(UseItemTransactionData $data) : bool{
		...
		switch($data->getActionType()){
			case UseItemTransactionData::ACTION_CLICK_BLOCK:
				...
				$now = microtime(true);
				$lastRightClickData = $this->lastRightClickData;
				$playerPosition = $data->getPlayerPosition();
				$spamBug = false;
				if($lastRightClickData !== null && $now - $this->lastRightClickTime < 0.1 && $lastRightClickData->getFace() === $data->getFace() && $lastRightClickData->getBlockPosition()->equals($data->getBlockPosition())){ //100ms
					$lastPlayerPosition = $lastRightClickData->getPlayerPosition();
					$playerDx = $lastPlayerPosition->x - $playerPosition->x;
					$playerDy = $lastPlayerPosition->y - $playerPosition->y;
					$playerDz = $lastPlayerPosition->z - $playerPosition->z;
					$lastClickPos = $lastRightClickData->getClickPosition();
					$clickDx = $lastClickPos->x - $clickPos->x;
					$clickDy = $lastClickPos->y - $clickPos->y;
					$clickDz = $lastClickPos->z - $clickPos->z;
					$spamBug = ($playerDx * $playerDx + $playerDy * $playerDy + $playerDz * $playerDz) < 0.00001 &&
						($clickDx * $clickDx + $clickDy * $clickDy + $clickDz * $clickDz) < 0.00001; //signature spam bug has 0 distance, but allow some error
				}
	...
	private function syncBlocksNearby(Vector3 $blockPos, ?int $face) : void{
		$playerPosition = $this->player->getLocation();
		$dx = $blockPos->x - $playerPosition->x;
		$dy = $blockPos->y - $playerPosition->y;
		$dz = $blockPos->z - $playerPosition->z;
		if(($dx * $dx + $dy * $dy + $dz * $dz) < 10000){
			$blocks = [];
			self::appendSidePositions($blocks, $blockPos);
			if($face !== null){
				$sidePos = $blockPos->getSide($face);
				self::appendSidePositions($blocks, $sidePos);
			}else{
				$blocks[] = $blockPos;
			}
	 ...
	 */
	private static function appendSidePositions(array &$blocks, Vector3 $pos) : void{
		$x = $pos->x;
		$y = $pos->y;
		$z = $pos->z;
		$blocks[] = new Vector3($x, $y - 1, $z);
		$blocks[] = new Vector3($x, $y + 1, $z);
		$blocks[] = new Vector3($x, $y, $z - 1);
		$blocks[] = new Vector3($x, $y, $z + 1);
		$blocks[] = new Vector3($x - 1, $y, $z);
		$blocks[] = new Vector3($x + 1, $y, $z);
	}
	...
	public function handleBlockActorData(BlockActorDataPacket $packet) : bool{
		...
		$playerPosition = $this->player->getLocation();
		$dx = $pos->x - $playerPosition->x;
		$dy = $pos->y - $playerPosition->y;
		$dz = $pos->z - $playerPosition->z;
		if(($dx * $dx + $dy * $dy + $dz * $dz) > 10000){
			return false;
		}

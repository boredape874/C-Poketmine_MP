...
final class AuthJwtHelper{
	...
	private static function validateAuthToken(string $jwt, string $signingKeyDer, ?string $issuer, string $audience, XboxAuthJwtBody|SelfSignedJwtBody $claims) : void{
		...
		if($claims instanceof SelfSignedJwtBody && isset($claimsArray["identity"]) && is_string($claimsArray["identity"])){
			if(!isset($claimsArray["leguuid"])){
				$claimsArray["leguuid"] = $claimsArray["identity"];
			}
			if(!isset($claimsArray["mid"])){
				$claimsArray["mid"] = $claimsArray["identity"];
			}
			unset($claimsArray["identity"]);
		}

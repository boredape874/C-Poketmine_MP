...
final class BlockTranslator{
	 ...
	 */
	private array $networkStateDataCache = [];
	...
	/**
	 * @param int[] $internalStateIds
	 * @phpstan-param list<int> $internalStateIds
	 * @return int[]
	 * @phpstan-return list<int>
	 */
	public function internalIdsToNetworkIds(array $internalStateIds) : array{
		$result = [];
		$networkIdCache = &$this->networkIdCache;
		$blockStateSerializer = $this->blockStateSerializer;
		$blockStateDictionary = $this->blockStateDictionary;
		$fallbackStateId = $this->fallbackStateId;
		foreach($internalStateIds as $internalStateId){
			if(isset($networkIdCache[$internalStateId])){
				$result[] = $networkIdCache[$internalStateId];
				continue;
			}

			try{
				$blockStateData = $blockStateSerializer->serialize($internalStateId);

				$networkId = $blockStateDictionary->lookupStateIdFromData($blockStateData);
				if($networkId === null){
					throw new AssumptionFailedError("Unmapped blockstate returned by blockstate serializer: " . $blockStateData->toNbt());
				}
			}catch(BlockStateSerializeException){
				$networkId = $fallbackStateId;
			}

			$networkIdCache[$internalStateId] = $networkId;
			$result[] = $networkId;
		}
	 ...
	 */
	public function &getNetworkIdCache() : array{
	...
	public function internalIdToNetworkStateData(int $internalStateId) : BlockStateData{
		if(isset($this->networkStateDataCache[$internalStateId])){
			return $this->networkStateDataCache[$internalStateId];
		}
		...
		return $this->networkStateDataCache[$internalStateId] = $this->blockStateDictionary->generateDataFromStateId($networkRuntimeId) ?? throw new AssumptionFailedError("We just looked up this state ID, so it must exist");
	}

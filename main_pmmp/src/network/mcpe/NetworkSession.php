...
class NetworkSession{
	 ...
	 */
	private array $sendBufferLengths = [];
	/**
	 * @var PromiseResolver[]
	...
	public function handleEncoded(string $payload) : void{
		...
		try{
			...
			$count = 0;
			try{
				PacketBatch::decodeRawStringCallback($decompressed, function(string $buffer) use (&$count) : bool{
	...
	public function addToSendBuffer(string $buffer) : void{
		...
		$this->sendBufferLengths[] = strlen($buffer);
	}

	private function flushGamePacketQueue() : void{
		$bufferCount = count($this->sendBuffer);
		if($bufferCount > 0){
			...
			try{
				...
				if($bufferCount === 1){
					PacketBatch::encodeRawSingle($stream, $this->sendBuffer[0], $this->sendBufferLengths[0]);
				}else{
					PacketBatch::encodeRawWithLengths($stream, $this->sendBuffer, $this->sendBufferLengths);
				}
	...
	public function prepareClientTranslatableMessage(Translatable $message) : array{
		...
		foreach($message->getParameters() as $parameter){
			$parameters[] = $parameter instanceof Translatable ? $language->translate($parameter) : $parameter;
		}
	...
	public function prefetchChunk(int $chunkX, int $chunkZ) : bool{
		if($this->player === null){
			return false;
		}
		return ChunkCache::getInstance($this->player->getLocation()->getWorld(), $this->compressor)->prefetchIfLoaded($chunkX, $chunkZ);
	}
	...
	public function syncPlayerList(array $players) : void{
		$entries = [];
		$skinAdapter = $this->typeConverter->getSkinAdapter();
		foreach($players as $player){
			$entries[] = PlayerListEntry::createAdditionEntry($player->getUniqueId(), $player->getId(), $player->getDisplayName(), $skinAdapter->toSkinData($player->getSkin()), $player->getXuid());
		}

...
final class CraftingDataCache{
		...
		foreach($manager->getCraftingRecipeIndex() as $index => $recipe){
			...
			if($recipe instanceof ShapelessRecipe){
				...
				$recipesWithTypeIds[] = new ProtocolShapelessRecipe(
					...
					$this->convertRecipeIngredients($recipe->getIngredientList(), $converter),
					$this->convertRecipeResults($recipe->getResults(), $converter),
					$nullUUID,
					$typeTag,
					50,
	...
	/**
	 * @phpstan-param list<\pocketmine\crafting\RecipeIngredient> $ingredients
	 * @phpstan-return list<\pocketmine\network\mcpe\protocol\types\recipe\RecipeIngredient>
	 */
	private function convertRecipeIngredients(array $ingredients, TypeConverter $converter) : array{
		$result = [];
		foreach($ingredients as $ingredient){
			$result[] = $converter->coreRecipeIngredientToNet($ingredient);
		}
		return $result;
	}

	/**
	 * @phpstan-param list<\pocketmine\item\Item> $outputs
	 * @phpstan-return list<\pocketmine\network\mcpe\protocol\types\inventory\ItemStack>
	 */
	private function convertRecipeResults(array $outputs, TypeConverter $converter) : array{
		$result = [];
		foreach($outputs as $output){
			$result[] = $converter->coreItemStackToNet($output);
		}
		return $result;
	}
}

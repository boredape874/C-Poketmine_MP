...
class ChunkCache implements ChunkListener{
		...
		if(!isset(self::$instances[$worldId])){
			...
			$world->addOnUnloadCallback(static function() use ($worldId) : void{
				foreach(self::$instances[$worldId] as $cache){
					\GlobalLogger::get()->info("PMMP_CHUNK_CACHE_STATS " . json_encode($cache->getStats(), JSON_THROW_ON_ERROR));
					$cache->caches = [];
				}
	...
	private int $prefetchAttempts = 0;
	private int $prefetchStarted = 0;
	private int $prefetchSkippedCached = 0;
	private int $prefetchSkippedUnloaded = 0;
	private int $prefetchPromiseHits = 0;
	private int $prefetchPacketHits = 0;
	/** @var true[] */
	private array $prefetchedChunkHashes = [];
	...
	private function prepareChunkAsync(int $chunkX, int $chunkZ, int $chunkHash, bool $prefetch = false) : CompressBatchPromise{
		...
		if($prefetch){
			...
			$this->prefetchedChunkHashes[$chunkHash] = true;
		}else{
			++$this->misses;
		}
	...
	public function request(int $chunkX, int $chunkZ) : CompressBatchPromise|string{
		...
		if(isset($this->caches[$chunkHash])){
			++$this->hits;
			if(isset($this->prefetchedChunkHashes[$chunkHash])){
				if(is_string($this->caches[$chunkHash])){
					++$this->prefetchPacketHits;
				}else{
					++$this->prefetchPromiseHits;
				}
			}
	...
	public function prefetchIfLoaded(int $chunkX, int $chunkZ) : bool{
		++$this->prefetchAttempts;
		$chunkHash = World::chunkHash($chunkX, $chunkZ);
		if(isset($this->caches[$chunkHash])){
			++$this->prefetchSkippedCached;
			return false;
		}
		if($this->world->getChunk($chunkX, $chunkZ) === null){
			++$this->prefetchSkippedUnloaded;
			return false;
		}

		$this->prepareChunkAsync($chunkX, $chunkZ, $chunkHash, true);
		return true;
	}
	...
	private function destroy(int $chunkX, int $chunkZ) : bool{
		...
		unset($this->prefetchedChunkHashes[$chunkHash]);
	...
	public function getStats() : array{
		return [
			"hits" => $this->hits,
			"misses" => $this->misses,
			"hit_percentage" => $this->getHitPercentage(),
			"cached_entries" => count($this->caches),
			"prefetch_attempts" => $this->prefetchAttempts,
			"prefetch_started" => $this->prefetchStarted,
			"prefetch_skipped_cached" => $this->prefetchSkippedCached,
			"prefetch_skipped_unloaded" => $this->prefetchSkippedUnloaded,
			"prefetch_promise_hits" => $this->prefetchPromiseHits,
			"prefetch_packet_hits" => $this->prefetchPacketHits,
		];
	}
}

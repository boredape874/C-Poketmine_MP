...
class FireworkRocket extends Entity implements Explosive, NeverSavedWithChunkEntity{
	...
	public function explode() : void{
		if(($explosionCount = count($this->explosions)) !== 0){
			...
			$world = $this->getWorld();
			$self = $this;
			$world->forEachCollidingEntity($this->getBoundingBox()->expandedCopy(5, 5, 5), static function(Entity $entity) use ($self, $world, $force) : void{
				if(!$entity instanceof Living){
					return;
				}

				$position = $entity->getPosition();
				$dx = $position->x - $self->location->x;
				$dy = $position->y - $self->location->y;
				$dz = $position->z - $self->location->z;
				...
				for($i = 0; $i < 2; $i++){
					...
					foreach(VoxelRayTrace::betweenPoints($self->location, $target) as $blockPos){
						if($world->getBlock($blockPos)->calculateIntercept($self->location, $target) !== null){
							continue 2; //obstruction, try another path
						}
					}

					//no obstruction
					$damage = $force * sqrt((5 - sqrt($distanceSquared)) / 5);
					$ev = new EntityDamageByEntityEvent($self, $entity, EntityDamageEvent::CAUSE_ENTITY_EXPLOSION, $damage);
					$entity->attack($ev);
					break;
				}
			}, $this);

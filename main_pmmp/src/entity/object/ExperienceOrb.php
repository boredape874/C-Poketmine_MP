...
class ExperienceOrb extends Entity{
	...
	protected function entityBaseTick(int $tickDiff = 1) : bool{
		...
		$maxTargetDistanceSquared = self::MAX_TARGET_DISTANCE * self::MAX_TARGET_DISTANCE;
		if($currentTarget !== null){
			$targetLocation = $currentTarget->location;
			$dx = $targetLocation->x - $this->location->x;
			$dy = $targetLocation->y - $this->location->y;
			$dz = $targetLocation->z - $this->location->z;
			if(!$currentTarget->isAlive() || !$currentTarget->getXpManager()->canAttractXpOrbs() || $dx * $dx + $dy * $dy + $dz * $dz > $maxTargetDistanceSquared){
				$currentTarget = null;
			}
		}
		...
		if($currentTarget !== null){
			...
			if($distance < 1){
				$distanceFactor = 1 - sqrt($distance);
				$this->motion = $this->motion->addVector($vector->normalize()->multiply(0.2 * $distanceFactor * $distanceFactor));
			}

...
class AreaEffectCloud extends Entity{
	...
	protected function entityBaseTick(int $tickDiff = 1) : bool{
		...
		if($this->age >= self::UPDATE_DELAY && ($this->age % self::UPDATE_DELAY) === 0){
			...
			$self = $this;
			$stopped = false;
			$this->getWorld()->forEachCollidingEntity($this->getBoundingBox(), static function(Entity $entity) use ($self, &$entities, &$radiusChange, &$maxAgeChange, &$stopped) : void{
				if($stopped){
					return;
				}
				if(!$entity instanceof Living || isset($self->victims[$entity->getId()])){
					return;
				}
				$entityPosition = $entity->getPosition();
				$xDiff = $entityPosition->getX() - $self->location->getX();
				$zDiff = $entityPosition->getZ() - $self->location->getZ();
				$radius = $self->radius;
				if(($xDiff * $xDiff + $zDiff * $zDiff) > $radius * $radius){
					return;
				}
				$entities[] = $entity;
				if($self->radiusChangeOnUse !== 0.0){
					$radiusChange += $self->radiusChangeOnUse;
					if($self->radius + $radiusChange <= 0){
						$stopped = true;
						return;
					}
				}
				if($self->maxAgeChangeOnUse !== 0){
					$maxAgeChange += $self->maxAgeChangeOnUse;
					if($self->maxAge + $maxAgeChange <= 0){
						$stopped = true;
					}
				}
			}, $this);

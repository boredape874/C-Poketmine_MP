...
class ItemEntity extends Entity{
	...
	protected function entityBaseTick(int $tickDiff = 1) : bool{
		...
		try{
			...
			if($this->hasMovementUpdate() && $this->isMergeCandidate() && $this->despawnDelay % self::MERGE_CHECK_PERIOD === 0){
				...
				$mergeTarget = $this;
				$self = $this;
				$this->getWorld()->forEachNearbyEntity($this->boundingBox->expandedCopy(0.5, 0.5, 0.5), static function(Entity $entity) use (&$mergeable, &$mergeTarget, $self) : void{
					if(!$entity instanceof ItemEntity || $entity->isFlaggedForDespawn()){
						return;
					}

					if($entity->isMergeable($self)){
						$mergeable ??= [$self]; //in case the merge target ends up not being this
						$mergeable[] = $entity;
	...
	public function onCollideWithPlayer(Player $player) : void{
		...
		$playerInventory = match(true){
			$offHandInventory->canAddStackedItem($item) => $offHandInventory,
			$inventory->getAddableItemQuantity($item) > 0 => $inventory,
			default => null
		};

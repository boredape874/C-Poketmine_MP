	...
	protected function onHitGround() : ?float{
		if($this->block instanceof Fallable){
			...
			if($damagePerBlock > 0 && ($fallenBlocks = round($this->fallDistance) - 1) > 0){
				...
				$self = $this;
				$this->getWorld()->forEachCollidingEntity($this->getBoundingBox(), static function(Entity $entity) use ($self, $damage) : void{
					if($entity instanceof Living){
						$ev = new EntityDamageByEntityEvent($self, $entity, EntityDamageEvent::CAUSE_FALLING_BLOCK, $damage);
						$entity->attack($ev);
					}
				});
			}

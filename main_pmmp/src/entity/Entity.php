...
abstract class Entity{
	...
	protected function updateMovement(bool $teleport = false) : void{
		$diffPositionX = $this->location->x - $this->lastLocation->x;
		$diffPositionY = $this->location->y - $this->lastLocation->y;
		$diffPositionZ = $this->location->z - $this->lastLocation->z;
		$diffPosition = $diffPositionX * $diffPositionX + $diffPositionY * $diffPositionY + $diffPositionZ * $diffPositionZ;
		$diffRotationYaw = $this->location->yaw - $this->lastLocation->yaw;
		$diffRotationPitch = $this->location->pitch - $this->lastLocation->pitch;
		$diffRotation = $diffRotationYaw * $diffRotationYaw + $diffRotationPitch * $diffRotationPitch;
	...
	protected function checkBlockIntersections() : void{
		...
		if($motionLengthSquared > 0){
			$multiplier = 0.014 / sqrt($motionLengthSquared);
			$this->motion = $this->motion->add($motionX * $multiplier, $motionY * $multiplier, $motionZ * $multiplier);
		}
	}
	...
	protected function sendSpawnPacket(Player $player) : void{
		...
		foreach($this->attributeMap->getAll() as $attr){
			$attributes[] = new NetworkAttribute($attr->getId(), $attr->getMinValue(), $attr->getMaxValue(), $attr->getValue(), $attr->getDefaultValue(), []);
		}

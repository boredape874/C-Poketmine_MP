...
class Arrow extends Projectile{
	...
	protected function onHitEntity(Entity $entityHit, RayTraceResult $hitResult) : void{
		...
		if($this->punchKnockback > 0){
			...
			$horizontalSpeed = sqrt($motionX * $motionX + $motionZ * $motionZ);
			if($horizontalSpeed > 0){
				$multiplier = $this->punchKnockback * 0.6 / $horizontalSpeed;
				$entityHit->setMotion($entityHit->getMotion()->add($motionX * $multiplier, 0.1, $motionZ * $multiplier));
			}
		}
	}
	...
	public function onCollideWithPlayer(Player $player) : void{
		...
		$playerInventory = match(true){
			...
			$inventory->canAddItem($item) => $inventory,
			default => null
		};

...
abstract class Projectile extends Entity{
	...
	protected function move(float $dx, float $dy, float $dz) : void{
		...
		$self = $this;
		$world->forEachCollidingEntity($this->boundingBox->addCoord($newDiff->x, $newDiff->y, $newDiff->z)->expand(1, 1, 1), static function(Entity $entity) use ($self, $start, &$end, &$hitResult, &$entityDistance) : void{
			if($entity->getId() === $self->getOwningEntityId() && $self->ticksLived < 5){
				return;
			}
			...
			$hitVector = $entityHitResult->hitVector;
			$dx = $self->location->x - $hitVector->x;
			$dy = $self->location->y - $hitVector->y;
			$dz = $self->location->z - $hitVector->z;

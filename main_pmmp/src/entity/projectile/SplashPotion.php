...
class SplashPotion extends Throwable{
	...
	protected function onHit(ProjectileHitEvent $event) : void{
		...
		if(count($effects) === 0){
			...
			$particle = new PotionSplashParticle(new Color(intdiv($r, $count), intdiv($g, $count), intdiv($b, $count), intdiv($a, $count)));
		}
		...
		if(!$this->willLinger()){
			if($hasEffects){
				$self = $this;
				$this->getWorld()->forEachCollidingEntity($this->boundingBox->expandedCopy(4.125, 2.125, 4.125), static function(Entity $entity) use ($self, $event) : void{
					if($entity instanceof Living){
						$eyePos = $entity->getEyePos();
						$dx = $eyePos->x - $self->location->x;
						$dy = $eyePos->y - $self->location->y;
						$dz = $eyePos->z - $self->location->z;
						...
						foreach($self->getPotionEffects() as $effect){
							...
							if(!($effect->getType() instanceof InstantEffect)){
								...
								$effect->getType()->applyEffect($entity, $effect, $distanceMultiplier, $self);
							}
						}
					}
				}, $this);

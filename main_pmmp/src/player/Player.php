...
class Player extends Human implements CommandSender, ChunkListener, IPlayer, NeverSavedWithChunkEntity{
	...
	private const CHUNK_PREFETCH_PER_ORDER = 8;
	private const CHUNK_POPULATION_PREFETCH_PER_ORDER = 2;
	...
	private int $usedChunkNeededCount = 0;
	private int $usedChunkRequestedGenerationCount = 0;
	private int $usedChunkRequestedSendingCount = 0;
	private int $usedChunkSentCount = 0;
	/**
	 * @var true[]
	...
	private float $chunkPrefetchDirectionX = 0.0;
	private float $chunkPrefetchDirectionZ = 0.0;
	private int $chunkPopulationPrefetchAttempts = 0;
	private int $chunkPopulationPrefetchStarted = 0;
	private int $chunkPopulationPrefetchSkippedBudget = 0;
	private int $chunkPopulationPrefetchSkippedNoDirection = 0;
	private int $chunkPopulationPrefetchSkippedActive = 0;
	private int $chunkPopulationPrefetchResolved = 0;
	private int $chunkPopulationPrefetchStale = 0;
	private int $chunkPopulationPrefetchGeneration = 0;
	/**
	 * @var int[] chunkHash => generation
	 * @phpstan-var array<int, int>
	 */
	private array $activeChunkPopulationPrefetches = [];
	...
	public function __construct(Server $server, NetworkSession $session, PlayerInfo $playerInfo, bool $authenticated, Location $spawnLocation, ?CompoundTag $namedtag){
		...
		$this->baseChunksPerTick = max(1, $this->server->getConfigGroup()->getPropertyInt(YmlServerProperties::CHUNK_SENDING_PER_TICK, 4));
		$this->maxChunksPerTick = max($this->baseChunksPerTick, min($this->baseChunksPerTick * 2, $this->baseChunksPerTick + 8));
		$this->chunksPerTick = $this->baseChunksPerTick;
		$spawnRadius = $this->server->getConfigGroup()->getPropertyInt(YmlServerProperties::CHUNK_SENDING_SPAWN_RADIUS, 4);
		$this->spawnThreshold = (int) ($spawnRadius * $spawnRadius * M_PI);
		...
		$this->setUsedChunkStatus(World::chunkHash($xSpawnChunk, $zSpawnChunk), UsedChunkStatus::NEEDED);
	...
	public function setViewDistance(int $distance) : void{
		...
		$spawnRadius = min($this->viewDistance, $this->server->getConfigGroup()->getPropertyInt(YmlServerProperties::CHUNK_SENDING_SPAWN_RADIUS, 4));
		$this->spawnThreshold = (int) ($spawnRadius * $spawnRadius * M_PI);
	...
	protected function unloadChunk(int $x, int $z, ?World $world = null) : void{
		...
		if(isset($this->usedChunks[$index])){
			$world->forEachChunkEntity($x, $z, function(Entity $entity) : void{
				if($entity !== $this){
					$entity->despawnFrom($this);
				}
			});
			$this->getNetworkSession()->stopUsingChunk($x, $z);
			$this->unsetUsedChunkStatus($index);
	...
	protected function spawnEntitiesOnChunk(int $chunkX, int $chunkZ) : void{
		$this->getWorld()->forEachChunkEntity($chunkX, $chunkZ, function(Entity $entity) : void{
	...
	private function updateAdaptiveChunksPerTick() : void{
		$queued = count($this->loadQueue);
		if($queued === 0){
			$this->chunksPerTick = $this->baseChunksPerTick;
			return;
		}

		$globalScale = $this->server->getChunkSendBudgetScale();
		if($globalScale < 100){
			$this->chunksPerTick = max(1, intdiv($this->baseChunksPerTick * $globalScale, 100));
			return;
		}

		$activeGeneration = count($this->activeChunkGenerationRequests);
		if($activeGeneration >= $this->baseChunksPerTick || $this->usedChunkRequestedSendingCount > $this->maxChunksPerTick){
			$this->chunksPerTick = max($this->baseChunksPerTick, $this->chunksPerTick - 1);
			return;
		}

		$maxChunksPerTick = $globalScale > 100 ? min($this->maxChunksPerTick, max($this->baseChunksPerTick, intdiv($this->maxChunksPerTick * $globalScale, 100))) : $this->baseChunksPerTick;
		if($queued > $this->chunksPerTick * 4 && $this->usedChunkRequestedSendingCount <= $this->baseChunksPerTick){
			$this->chunksPerTick = min($maxChunksPerTick, $this->chunksPerTick + 1);
		}
	}
	...
	protected function requestChunks() : void{
		...
		$this->updateAdaptiveChunksPerTick();
		...
		foreach($this->loadQueue as $index => $distance){
			...
			$this->setUsedChunkStatus($index, UsedChunkStatus::REQUESTED_GENERATION);
			...
			$world->requestChunkPopulation($X, $Z, $this->chunkLoader)->onCompletion(
				function() use ($X, $Z, $index, $world) : void{
					...
					unset($this->activeChunkGenerationRequests[$index]);
					$this->setUsedChunkStatus($index, UsedChunkStatus::REQUESTED_SENDING);

					$this->getNetworkSession()->startUsingChunk($X, $Z, function() use ($X, $Z, $index) : void{
						$this->setUsedChunkStatus($index, UsedChunkStatus::SENT);
	...
	private function prefetchQueuedChunkPackets() : void{
		if(count($this->loadQueue) === 0){
			return;
		}

		$prefetched = 0;
		$limit = min(self::CHUNK_PREFETCH_PER_ORDER, max(1, $this->chunksPerTick * 2));
		...
		$directionX = $this->chunkPrefetchDirectionX;
		$directionZ = $this->chunkPrefetchDirectionZ;
		$hasDirection = $directionX !== 0.0 || $directionZ !== 0.0;
		if($hasDirection){
			foreach($this->loadQueue as $hash => $_){
				World::getXZ($hash, $chunkX, $chunkZ);
				$offsetX = $chunkX - $centerChunkX;
				$offsetZ = $chunkZ - $centerChunkZ;
				if($offsetX * $directionX + $offsetZ * $directionZ < 0.0){
					continue;
				}
				if($networkSession->prefetchChunk($chunkX, $chunkZ) && ++$prefetched >= $limit){
					return;
				}
			}
		}
		foreach($this->loadQueue as $hash => $_){
			World::getXZ($hash, $chunkX, $chunkZ);
			if($hasDirection && ($chunkX - $centerChunkX) * $directionX + ($chunkZ - $centerChunkZ) * $directionZ >= 0.0){
				continue;
			}
			if($networkSession->prefetchChunk($chunkX, $chunkZ) && ++$prefetched >= $limit){
				return;
	...
	private function prefetchAheadChunkPopulation() : void{
		if(count($this->loadQueue) === 0){
			return;
		}
		...
		$globalScale = $this->server->getChunkSendBudgetScale();
		if($globalScale < 100){
			++$this->chunkPopulationPrefetchSkippedBudget;
			return;
		}
		...
		if($directionX === 0.0 && $directionZ === 0.0){
			++$this->chunkPopulationPrefetchSkippedNoDirection;
			return;
		}

		$centerChunkX = $this->location->getFloorX() >> Chunk::COORD_BIT_SIZE;
		$centerChunkZ = $this->location->getFloorZ() >> Chunk::COORD_BIT_SIZE;
		$stepX = $directionX > 0.0 ? 1 : ($directionX < 0.0 ? -1 : 0);
		$stepZ = $directionZ > 0.0 ? 1 : ($directionZ < 0.0 ? -1 : 0);
		$radius = $this->server->getAllowedViewDistance($this->viewDistance) + 1;
		$world = $this->getWorld();
		$generation = $this->chunkPopulationPrefetchGeneration;
		$started = 0;
		for($i = 0; $i < self::CHUNK_POPULATION_PREFETCH_PER_ORDER; ++$i){
			$chunkX = $centerChunkX + $stepX * $radius + ($stepZ !== 0 ? $i - 1 : 0);
			$chunkZ = $centerChunkZ + $stepZ * $radius + ($stepX !== 0 ? $i - 1 : 0);
			$hash = World::chunkHash($chunkX, $chunkZ);
			if(isset($this->usedChunks[$hash]) || isset($this->loadQueue[$hash])){
				continue;
			}
			if(isset($this->activeChunkPopulationPrefetches[$hash])){
				++$this->chunkPopulationPrefetchSkippedActive;
				continue;
			}
			++$this->chunkPopulationPrefetchAttempts;
			$this->activeChunkPopulationPrefetches[$hash] = $generation;
			$world->requestChunkPopulation($chunkX, $chunkZ, null)->onCompletion(
				function() use ($hash, $generation, $world) : void{
					if(($this->activeChunkPopulationPrefetches[$hash] ?? null) === $generation){
						unset($this->activeChunkPopulationPrefetches[$hash]);
					}
					if(!$this->isConnected() || $world !== $this->getWorld() || $this->chunkPopulationPrefetchGeneration !== $generation){
						++$this->chunkPopulationPrefetchStale;
						return;
					}
					++$this->chunkPopulationPrefetchResolved;
				},
				function() use ($hash, $generation) : void{
					if(($this->activeChunkPopulationPrefetches[$hash] ?? null) === $generation){
						unset($this->activeChunkPopulationPrefetches[$hash]);
					}
					++$this->chunkPopulationPrefetchStale;
			...
			++$this->chunkPopulationPrefetchStarted;
			if(++$started >= self::CHUNK_POPULATION_PREFETCH_PER_ORDER){
				return;
			}
		}
	}
	...
	protected function orderChunks() : void{
		...
		$this->prefetchQueuedChunkPackets();
		$this->prefetchAheadChunkPopulation();
	...
	private function incrementUsedChunkStatusCounter(UsedChunkStatus $status) : void{
		if($status === UsedChunkStatus::NEEDED){
			++$this->usedChunkNeededCount;
		}elseif($status === UsedChunkStatus::REQUESTED_GENERATION){
			++$this->usedChunkRequestedGenerationCount;
		}elseif($status === UsedChunkStatus::REQUESTED_SENDING){
			++$this->usedChunkRequestedSendingCount;
		}else{
			++$this->usedChunkSentCount;
		}
	}

	private function decrementUsedChunkStatusCounter(UsedChunkStatus $status) : void{
		if($status === UsedChunkStatus::NEEDED){
			--$this->usedChunkNeededCount;
		}elseif($status === UsedChunkStatus::REQUESTED_GENERATION){
			--$this->usedChunkRequestedGenerationCount;
	...
	private function setUsedChunkStatus(int $hash, UsedChunkStatus $status) : void{
		$oldStatus = $this->usedChunks[$hash] ?? null;
		if($oldStatus === $status){
			return;
		}
		if($oldStatus !== null){
			$this->decrementUsedChunkStatusCounter($oldStatus);
		}
		$this->usedChunks[$hash] = $status;
		$this->incrementUsedChunkStatusCounter($status);
	}

	private function unsetUsedChunkStatus(int $hash) : void{
		$oldStatus = $this->usedChunks[$hash] ?? null;
		if($oldStatus === null){
			return;
		}
		unset($this->usedChunks[$hash]);
		$this->decrementUsedChunkStatusCounter($oldStatus);
	}

	private function clearUsedChunks() : void{
		$this->usedChunks = [];
		$this->usedChunkNeededCount = 0;
		$this->usedChunkRequestedGenerationCount = 0;
		$this->usedChunkRequestedSendingCount = 0;
		$this->usedChunkSentCount = 0;
		$this->activeChunkPopulationPrefetches = [];
		++$this->chunkPopulationPrefetchGeneration;
	}

	/**
	 * @phpstan-return array{queued: int, needed: int, requested_generation: int, active_generation: int, requested_sending: int, sent: int, pending: int, used_total: int, chunks_per_tick: int, base_chunks_per_tick: int, max_chunks_per_tick: int, population_prefetch_attempts: int, population_prefetch_started: int, population_prefetch_skipped_budget: int, population_prefetch_skipped_no_direction: int, population_prefetch_skipped_active: int, population_prefetch_resolved: int, population_prefetch_stale: int, population_prefetch_active: int}
	 */
	public function getChunkRequestPressure() : array{
		$queued = count($this->loadQueue);
		$activeGeneration = count($this->activeChunkGenerationRequests);
		return [
			'queued' => $queued,
			'needed' => $this->usedChunkNeededCount,
			'requested_generation' => $this->usedChunkRequestedGenerationCount,
			'active_generation' => $activeGeneration,
			'requested_sending' => $this->usedChunkRequestedSendingCount,
			'sent' => $this->usedChunkSentCount,
			'pending' => $queued + $activeGeneration + $this->usedChunkRequestedSendingCount,
			'used_total' => $this->usedChunkNeededCount + $this->usedChunkRequestedGenerationCount + $this->usedChunkRequestedSendingCount + $this->usedChunkSentCount,
			'chunks_per_tick' => $this->chunksPerTick,
			'base_chunks_per_tick' => $this->baseChunksPerTick,
			'max_chunks_per_tick' => $this->maxChunksPerTick,
			'population_prefetch_attempts' => $this->chunkPopulationPrefetchAttempts,
			'population_prefetch_started' => $this->chunkPopulationPrefetchStarted,
			'population_prefetch_skipped_budget' => $this->chunkPopulationPrefetchSkippedBudget,
			'population_prefetch_skipped_no_direction' => $this->chunkPopulationPrefetchSkippedNoDirection,
			'population_prefetch_skipped_active' => $this->chunkPopulationPrefetchSkippedActive,
			'population_prefetch_resolved' => $this->chunkPopulationPrefetchResolved,
			'population_prefetch_stale' => $this->chunkPopulationPrefetchStale,
			'population_prefetch_active' => count($this->activeChunkPopulationPrefetches),
		];
	}
	...
	protected function checkNearEntities() : void{
		$this->getWorld()->forEachNearbyEntity($this->boundingBox->expandedCopy(1, 0.5, 1), function(Entity $entity) : void{
	...
	protected function processMostRecentMovements() : void{
		...
		$deltaX = $location->x - $lastLocation->x;
		$deltaY = $location->y - $lastLocation->y;
		$deltaZ = $location->z - $lastLocation->z;
		$delta = $deltaX * $deltaX + $deltaY * $deltaY + $deltaZ * $deltaZ;
		$deltaAngle = abs($lastLocation->yaw - $location->yaw) + abs($lastLocation->pitch - $location->pitch);

		if($delta > 0.0001 || $deltaAngle > 1.0){
			$to = clone $location;

			if(PlayerMoveEvent::hasHandlers()){
				$from = clone $lastLocation;
				...
				$evTo = $ev->getTo();
				$evDeltaX = $to->x - $evTo->x;
				$evDeltaY = $to->y - $evTo->y;
				$evDeltaZ = $to->z - $evTo->z;
				if($evDeltaX * $evDeltaX + $evDeltaY * $evDeltaY + $evDeltaZ * $evDeltaZ > 0.01){ //If plugins modify the destination
					$this->teleport($evTo);
					return;
				}
			}
			...
			if($horizontalDistanceSquared > 0){
				if(($this->chunkPrefetchDirectionX !== 0.0 || $this->chunkPrefetchDirectionZ !== 0.0) && $this->chunkPrefetchDirectionX * $deltaX + $this->chunkPrefetchDirectionZ * $deltaZ <= 0.0){
					++$this->chunkPopulationPrefetchGeneration;
				}
				$this->chunkPrefetchDirectionX = $deltaX;
				$this->chunkPrefetchDirectionZ = $deltaZ;
				$horizontalDistanceTravelled = sqrt($horizontalDistanceSquared);
	...
	public function continueBreakBlock(Vector3 $pos, int $face) : void{
		if($this->blockBreakHandler !== null){
			$blockPos = $this->blockBreakHandler->getBlockPos();
			$dx = $blockPos->x - $pos->x;
			$dy = $blockPos->y - $pos->y;
			$dz = $blockPos->z - $pos->z;
	...
	protected function onDeath() : void{
		...
		if(!$ev->getKeepInventory()){
			...
			$clearInventory = static function(Inventory $inventory) : void{
				$kept = [];
	...
	public function onChunkChanged(int $chunkX, int $chunkZ, Chunk $chunk) : void{
		...
		if($status === UsedChunkStatus::SENT){
			$this->setUsedChunkStatus($hash, UsedChunkStatus::NEEDED);
			$this->nextChunkOrderRun = 0;
		}
	}

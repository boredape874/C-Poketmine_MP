...
class SimpleInventory extends BaseInventory{
	...
	public function getAddableItemQuantity(Item $item) : int{
		...
		for($i = 0, $size = $this->slots->getSize(); $i < $size; ++$i){
			$slotItem = $this->slots[$i];
			if($slotItem === null){
				$count -= $maxStackSize;
			}elseif($slotItem->equals($item, true, true) && ($diff = $maxStackSize - $slotItem->getCount()) > 0){
				$count -= $diff;
			}
	...
	public function addItem(Item ...$slots) : array{
		$returnSlots = [];
		$maxInventoryStackSize = $this->getMaxStackSize();
		...
		foreach($slots as $slot){
			...
			$newItem = clone $slot;
			$maxStackSize = min($maxInventoryStackSize, $newItem->getMaxStackSize());

			for($i = 0, $size = $this->slots->getSize(); $i < $size; ++$i){
				$slotItem = $this->slots[$i];
				if($slotItem === null || !$slotItem->equals($newItem, true, true)){
					continue;
				}

				$slotCount = $slotItem->getCount();
				...
				$oldItem = clone $slotItem;
				$newItem->setCount($newItem->getCount() - $amount);
				$slotItem->setCount($slotCount + $amount);
				$this->onSlotChange($i, $oldItem);
			...
			for($i = 0, $size = $this->slots->getSize(); $i < $size; ++$i){
				...
				$this->onSlotChange($i, VanillaItems::AIR());
				if($newItem->getCount() <= 0){
					continue 2;
				}
			}

...
class World implements ChunkManager{
	...
	private int $chunkPopulationRequests = 0;
	private int $chunkPopulationSharedHits = 0;
	private int $chunkPopulationQueued = 0;
	private int $chunkPopulationQueueDuplicates = 0;
	private int $chunkPopulationStarted = 0;
	private int $chunkPopulationResolved = 0;
	private int $chunkPopulationRejected = 0;
	...
	/**
	 * @phpstan-return array{requests: int, shared_hits: int, queued: int, queue_duplicates: int, started: int, resolved: int, rejected: int, active: int, pending: int, queued_unique: int}
	 */
	public function getChunkPopulationStats() : array{
		return [
			'requests' => $this->chunkPopulationRequests,
			'shared_hits' => $this->chunkPopulationSharedHits,
			'queued' => $this->chunkPopulationQueued,
			'queue_duplicates' => $this->chunkPopulationQueueDuplicates,
			'started' => $this->chunkPopulationStarted,
			'resolved' => $this->chunkPopulationResolved,
			'rejected' => $this->chunkPopulationRejected,
			'active' => count($this->activeChunkPopulationTasks),
			'pending' => count($this->chunkPopulationRequestMap),
			'queued_unique' => count($this->chunkPopulationRequestQueueIndex),
		];
	}
	...
	public function onUnload() : void{
		...
		$this->logger->info("PMMP_CHUNK_POPULATION_STATS " . json_encode($this->getChunkPopulationStats(), JSON_THROW_ON_ERROR));
	...
	protected function actuallyDoTick(int $currentTick) : void{
		...
		while($this->neighbourBlockUpdateQueue->count() > 0){
			...
			$this->forEachNearbyEntity(AxisAlignedBB::one()->offset($x, $y, $z), static function(Entity $entity) : void{
			...
			if(count($this->players) > 0){
				foreach($this->changedBlocks as $index => $blocks){
					...
					if(count($blocks) > 512){
						...
						$chunkPlayers = $this->playerChunkListeners[$index] ?? [];
						if(count($chunkPlayers) > 0){
							foreach($chunkPlayers as $p){
	...
	public function createBlockUpdatePackets(array $blocks) : array{
		...
		foreach($blocks as $b){
			...
			$y = (int) $b->y;
			$z = (int) $b->z;
			$fullBlock = $this->getBlockAt($x, $y, $z);
			$fullBlockStateId = $fullBlock->getStateId();
			...
			if($expectedClass !== null){
				...
				if($tile instanceof Spawnable && $tile instanceof $expectedClass && count($fakeStateProperties = $tile->getRenderUpdateBugWorkaroundStateProperties($fullBlock)) > 0){
					$originalStateData = $blockTranslator->internalIdToNetworkStateData($fullBlockStateId);
					$states = $originalStateData->getStates();
					foreach(Utils::stringifyKeys($fakeStateProperties) as $name => $value){
						$states[$name] = $value;
					}
			...
			$packets[] = UpdateBlockPacket::create(
				$blockPosition,
				$blockTranslator->internalIdToNetworkId($fullBlockStateId),
				UpdateBlockPacket::FLAG_NETWORK,
				UpdateBlockPacket::DATA_LAYER_NORMAL
			);
	...
	private function tickChunk(int $chunkX, int $chunkZ) : void{
		...
		foreach($this->entitiesByChunk[World::chunkHash($chunkX, $chunkZ)] ?? [] as $entity){
		...
		try{
			foreach($this->chunks as $chunkHash => $chunk){
				...
				$this->provider->saveChunk($chunkX, $chunkZ, new ChunkData(
					$chunk->getSubChunks(),
					$chunk->isPopulated(),
					$this->createChunkEntityNbt($chunkX, $chunkZ),
					$this->createChunkTileNbt($chunk),
	...
	/**
	 * @phpstan-return list<\pocketmine\nbt\tag\CompoundTag>
	 */
	private function createChunkEntityNbt(int $chunkX, int $chunkZ) : array{
		$result = [];
		foreach($this->entitiesByChunk[World::chunkHash($chunkX, $chunkZ)] ?? [] as $entity){
			if($entity->canSaveWithChunk()){
				$result[] = $entity->saveNBT();
			}
		}
	 ...
	 * @phpstan-return list<\pocketmine\nbt\tag\CompoundTag>
	 */
	private function createChunkTileNbt(\pocketmine\world\format\Chunk $chunk) : array{
		$result = [];
	...
	public function useItemOn(Vector3 $vector, Item &$item, int $face, ?Vector3 $clickVector = null, ?Player $player = null, bool $playSound = false, array &$returnedItems = []) : bool{
		...
		foreach($tx->getBlocks() as [$x, $y, $z, $block]){
			...
			foreach($block->getCollisionBoxes() as $collisionBox){
				if($this->hasCollidingEntities($collisionBox)){
	...
	public function getCollidingEntities(AxisAlignedBB $bb, ?Entity $entity = null) : array{
		...
		$this->forEachCollidingEntity($bb, static function(Entity $ent) use (&$nearby) : void{
			$nearby[] = $ent;
		}, $entity);
	...
	/**
	 * Returns whether any collidable entity bounding box intersects the given bounding box.
	 */
	public function hasCollidingEntities(AxisAlignedBB $bb, ?Entity $entity = null) : bool{
		...
		for($x = $minX; $x <= $maxX; ++$x){
			for($z = $minZ; $z <= $maxZ; ++$z){
				foreach($this->entitiesByChunk[World::chunkHash($x, $z)] ?? [] as $ent){
					if(
						$ent !== $entity &&
						$ent->boundingBox->intersectsWith($bb) &&
						$ent->canBeCollidedWith() &&
						($entity === null || $entity->canCollideWith($ent))
					){
						return true;
					}
				}
	...
	/**
	 * @phpstan-param \Closure(Entity) : void $callback
	 */
	public function forEachCollidingEntity(AxisAlignedBB $bb, \Closure $callback, ?Entity $entity = null) : void{
		$this->forEachNearbyEntity($bb, static function(Entity $ent) use ($callback, $entity) : void{
	...
	public function getNearbyEntities(AxisAlignedBB $bb, ?Entity $entity = null) : array{
		...
		$this->forEachNearbyEntity($bb, static function(Entity $ent) use (&$nearby) : void{
			$nearby[] = $ent;
		}, $entity);
	...
	/**
	 * Returns whether any entity bounding box intersects the given bounding box, excluding the given entity.
	 */
	public function hasNearbyEntities(AxisAlignedBB $bb, ?Entity $entity = null) : bool{
		$minX = ((int) floor($bb->minX - 2)) >> Chunk::COORD_BIT_SIZE;
		$maxX = ((int) floor($bb->maxX + 2)) >> Chunk::COORD_BIT_SIZE;
		$minZ = ((int) floor($bb->minZ - 2)) >> Chunk::COORD_BIT_SIZE;
		$maxZ = ((int) floor($bb->maxZ + 2)) >> Chunk::COORD_BIT_SIZE;
	 ...
	 * @phpstan-param \Closure(Entity) : void $callback
	 */
	public function forEachNearbyEntity(AxisAlignedBB $bb, \Closure $callback, ?Entity $entity = null) : void{
		$minX = ((int) floor($bb->minX - 2)) >> Chunk::COORD_BIT_SIZE;
		$maxX = ((int) floor($bb->maxX + 2)) >> Chunk::COORD_BIT_SIZE;
		$minZ = ((int) floor($bb->minZ - 2)) >> Chunk::COORD_BIT_SIZE;
		$maxZ = ((int) floor($bb->maxZ + 2)) >> Chunk::COORD_BIT_SIZE;
	...
	public function getNearestEntity(Vector3 $pos, float $maxDistance, string $entityType = Entity::class, bool $includeDead = false) : ?Entity{
		...
		for($x = $minX; $x <= $maxX; ++$x){
			for($z = $minZ; $z <= $maxZ; ++$z){
				$entities = $this->entitiesByChunk[World::chunkHash($x, $z)] ?? [];
				foreach($entities as $entity){
					if(!($entity instanceof $entityType) || $entity->isFlaggedForDespawn() || (!$includeDead && !$entity->isAlive())){
						continue;
					}
					$entityPosition = $entity->getPosition();
					$dx = $entityPosition->x - $posX;
					$dy = $entityPosition->y - $posY;
					$dz = $entityPosition->z - $posZ;
	 ...
	 * @internal
	 *
	 * @phpstan-param \Closure(Entity) : void $visitor
	 */
	public function forEachChunkEntity(int $chunkX, int $chunkZ, \Closure $visitor) : void{
		$chunkHash = World::chunkHash($chunkX, $chunkZ);
		if(!isset($this->entitiesByChunk[$chunkHash])){
			return;
		}
		foreach($this->entitiesByChunk[$chunkHash] as $entity){
			$visitor($entity);
		}
	}
	...
	public function setChunk(int $chunkX, int $chunkZ, Chunk $chunk) : void{
		...
		for($cX = -1; $cX <= 1; ++$cX){
			for($cZ = -1; $cZ <= 1; ++$cZ){
				foreach($this->entitiesByChunk[World::chunkHash($chunkX + $cX, $chunkZ + $cZ)] ?? [] as $entity){
					$entity->onNearbyBlockChange();
				}
			}
		}
	...
	public function unloadChunk(int $x, int $z, bool $safe = true, bool $trySave = true) : bool{
		...
		if($chunk !== null){
			...
			foreach($this->entitiesByChunk[World::chunkHash($x, $z)] ?? [] as $entity){
				if($entity instanceof Player){
					continue;
				}
				$entity->close();
	...
	private function addChunkHashToPopulationRequestQueue(int $chunkHash) : void{
		if(!isset($this->chunkPopulationRequestQueueIndex[$chunkHash])){
			...
			++$this->chunkPopulationQueued;
		}else{
			++$this->chunkPopulationQueueDuplicates;
		}
	}

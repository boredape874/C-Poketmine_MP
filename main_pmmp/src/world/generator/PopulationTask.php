...
class PopulationTask extends AsyncTask{
	...
	public function __construct(
		...
		$serialAdjacentChunks = [];
		foreach($adjacentChunks as $relativeChunkHash => $c){
			$serialAdjacentChunks[$relativeChunkHash] = $c !== null ? FastChunkSerializer::serializeTerrain($c) : null;
		}
		$this->adjacentChunks = igbinary_serialize($serialAdjacentChunks) ?? throw new AssumptionFailedError("igbinary_serialize() returned null");
	...
	public function onRun() : void{
		...
		$chunks = [];
		foreach($serialChunks as $relativeChunkHash => $serialized){
			if($serialized === null){
				$chunks[$relativeChunkHash] = null;
				continue;
			}
			$deserializedChunk = FastChunkSerializer::deserializeTerrain($serialized);
			$deserializedChunk->clearTerrainDirtyFlags(); //this allows us to avoid sending existing chunks back to the main thread if they haven't changed during generation
			$chunks[$relativeChunkHash] = $deserializedChunk;
		}

...
class Explosion{
			...
			for($j = 0; $j < $this->rays; ++$j){
				for($k = 0; $k < $this->rays; ++$k){
					if($i === 0 || $i === $mRays || $j === 0 || $j === $mRays || $k === 0 || $k === $mRays){
						...
						$len = sqrt($shiftX * $shiftX + $shiftY * $shiftY + $shiftZ * $shiftZ);
	...
	public function explodeB() : bool{
		...
		$sourceY = $source->y;
		$sourceZ = $source->z;
		$explosionSizeSquared = $explosionSize * $explosionSize;
		$this->world->forEachNearbyEntity($explosionBB, static function(Entity $entity) use ($self, $what, $source, $sourceX, $sourceY, $sourceZ, $explosionSize, $explosionSizeSquared) : void{
			$entityPos = $entity->getPosition();
			$dx = $entityPos->x - $sourceX;
			$dy = $entityPos->y - $sourceY;
			$dz = $entityPos->z - $sourceZ;
			$distanceSquared = $dx * $dx + $dy * $dy + $dz * $dz;

			if($distanceSquared <= $explosionSizeSquared){
				$rawDistance = sqrt($distanceSquared);
				$distance = $rawDistance / $explosionSize;
				$motion = $rawDistance > 0 ? new Vector3($dx / $rawDistance, $dy / $rawDistance, $dz / $rawDistance) : new Vector3(0, 0, 0);
				$exposure = $self->getExposure($source, $entity);
		...
		}, $what instanceof Entity ? $what : null);
